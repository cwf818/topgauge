...
- **v0.9.1** — kimi user plugin interval mappings corrected against the captured `quota.real.kimi.json` (shortInterval ← `usages.limits[0].detail` with `intervalMs = window.duration` MINUTES × 60s × 1000; midInterval ← `usages.detail` with fixed 1-week `intervalMs`; longInterval ← `totalQuota.remaining` with all 3 time fields null). `longInterval.usedPercent` reads `totalQuota.used` directly (was 100−used = inverted). Dropped 11 dead-code symbols from `src/{render,api,providers,diagnostics,status-store,git-info,quotes,dispatch}.ts` in 3 classes (A: 0-caller test hooks; B: `*WithKind` legacy twins; C: 8 internal-only type/const demotions). Surviving callers/published symbols unchanged.
...
- **v0.9.0** — full rename to `topgauge` (ToPGauge). Package id, marketplace id, plugin name, plugin key (`topgauge@topgauge`), state-dir path, settings.json marker, all stderr banners, and the docs are renamed. Provider strings (`minimax`, `deepseek`) are unchanged. The legacy `topgauge-cc` and `tokenplan-usage-hud` plugin names are NOT recognized — users upgrading from a pre-rename install should run `:uninstall` against the old name first (or wipe by hand), then re-install.
- **v0.7.0** — renamed from `tokenplan-usage-hud` to `topgauge-cc` (ToPGauge-CC). Superseded by v0.9.0 below.
