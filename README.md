...
| `m_windowMemUsage` | System RAM used bar + 5-band-colored percentage, e.g. `▓▓▓▓▓░░░ 62%`. Parallel of `m_windowContext` (which renders the context-window bar+percent from stdin). | `os.totalmem()` / `os.freemem()` (Darwin: `vm_stat`) | agnostic | `color`, `display`, `nulldrop` | v0.8.36+. 5-band colored via `percentBands` (same helper as `m_window\|term\|short`). `\|color\|<c>` overrides the band color. `\|display\|<used\|remaining>` selects which side of the bar is colored. No label prefix (parallel of `m_windowContext`). Opt-in — not in default `lineTemplate`. |
