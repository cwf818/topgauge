...
5. Render: `src/render.ts` emits a single compact line `Usage: 5h ░░░▓▓▓▓▓ 38%(47m🕒) · wk ░░░░░▓▓▓ 39%(4d47m🕓)`. Layout: a single mode label prefix (`Usage:` or `Remain:`), then per-window `<label> <leftPlain><rightColored> <coloredN%><RESET>(<reset><glyph>)` segments joined with ` · `. Default mode is **`used`** (line begins with `Usage:`); set `display: "remaining"` in `config.json` to switch. 5-band colors (256-color SGR): bright green / dark green / yellow / orange / red, applied to the displayed value at 0/20/40/60/80 boundaries. The colored chunk is always on the right side of the bar, sized by the displayed value. The reset countdown's glyph comes from `stale.resetArrows` (default 12 clock-face emoji `🕛…🕚`), indexed by `remainingMs / resetDurationMs` so the array reads left-to-right as "fresh → about to reset". Two glyphs (`["⏳","⌛"]`) reproduce the v0.2.1 hourglass pair; one glyph is static. Providers without start data (DeepSeek, legacy) fall back to index 0.
