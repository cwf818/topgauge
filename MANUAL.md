...
| `m_memUsageStatus`                 | System RAM usage, `Mem:15.9G/63.7G` (default label `Mem:`).                          | `os.totalmem()` / `os.freemem()` (Darwin: `vm_stat`)      | agnostic           | `color`, `nulldrop`                    | v0.8.17+. Cross-platform live sample; no caching. 1024-base, G uses `.toFixed(1)`, M/K use `.toFixed(0)`. Query failure → `Mem:n/a` placeholder. Label is user-configurable via `labels.labelMemUsage` (default `"Mem:"`, mirrors ccstatusline). |
